...
public class Scalebar: UIView {
  ...
  private func sharedInitialization() {
    updateCoalescer = Coalescer(dispatchQueue: DispatchQueue.main, interval: DispatchTimeInterval.milliseconds(500), action: updateScaleDisplayIfNecessary)

    isUserInteractionEnabled = false
    isOpaque = false

    units = NSLocale.current.usesMetricSystem ? .metric : .imperial
  ...
  public override var intrinsicContentSize: CGSize {
    if let renderer = renderer {
      if maximumIntrinsicWidth > 0 {
        ...
        return CGSize(width: UIViewNoIntrinsicMetric, height: renderer.displayHeight)
      }
    } else {
...
internal extension ScalebarRenderer {
  ...
  func drawText(text: String, frame: CGRect, alignment: NSTextAlignment) {
    ...
    if let shadowColor = scalebar.textShadowColor {
      let shadowAttrs = [
        NSAttributedStringKey.font: scalebar.font,
  ...
  func drawSegmentsText(segmentInfos: [SegmentInfo], scaleDisplay: ScaleDisplay, startingX: CGFloat, textY: CGFloat) {
    ...
    let zeroTextFrame = CGRect(
      x: startingX - endOffset,
      y: textY,
      width: scalebar.zeroStringWidth,
      height: scalebar.fontHeight
    )

    drawText(text: "0", frame: zeroTextFrame, alignment: .left)
    ...
    for si in segmentInfos {
      if si.index == segmentInfos.last?.index {
        ...
        drawText(text: si.text, frame: segmentTextFrame, alignment: .right)
        ...
        let unitsTextFrame = CGRect(
          x: segmentTextFrame.maxX,
          y: textY,
          width: unitsTextWidth,
          height: scalebar.fontHeight
        )

        drawText(text: unitsText, frame: unitsTextFrame, alignment: .right)
        ...
        let segmentTextFrame = CGRect(
          x: segmentX - (si.textWidth / 2),
          y: textY,
          width: si.textWidth,
          height: scalebar.fontHeight
        )

        drawText(text: si.text, frame: segmentTextFrame, alignment: .center)
      }
    }
  }
}
...
internal class ScalebarLineStyleRenderer: ScalebarRenderer {
  ...
  func draw(rect: CGRect) {
    ...
    drawText(text: text, frame: textFrame, alignment: .center)
...
internal class ScalebarDualUnitLineStyleRenderer: ScalebarRenderer {
  ...
  func draw(rect: CGRect) {
    ...
    let topTextFrame = CGRect(
      x: lineX + lineScreenLength + halfLineWidth - topTextMapLengthStringWidth,
      y: y,
      width: topTextWidth,
      height: scalebar.fontHeight
    )

    drawText(text: topText, frame: topTextFrame, alignment: .right)
    ...
    if let numberString = Scalebar.numberFormatter.string(from: NSNumber(value: otherLineMapLength)) {
      ...
      let bottomTextFrame = CGRect(
        x: lineX + otherLineScreenLength + halfLineWidth - bottomTextNumberStringWidth,
        y: lineY + Scalebar.tick2Height + Scalebar.labelYPad,
        width: bottomTextWidth,
        height: scalebar.fontHeight
      )

      drawText(text: bottomText, frame: bottomTextFrame, alignment: .right)
    }
